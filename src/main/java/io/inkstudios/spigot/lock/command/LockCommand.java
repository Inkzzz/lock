...
public class LockCommand implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		...
		if (state != null && state == LockState.LOCK) {
			...
			player.sendMessage(ChatUtil.toColour("&aYou have removed your 'lock' status."));
			return true;
		}
		
		account.setState(LockState.LOCK);
		player.sendMessage(ChatUtil.toColour("&aYou have set your lock state to 'lock'."));
